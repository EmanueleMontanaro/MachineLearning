...
def loadD():
    D = numpy.loadtxt('iris.csv',usecols = range (0,4), delimiter=',',dtype=numpy.float32).T
    return D
    
def loadL():
    L = numpy.loadtxt('iris.csv', usecols = 4, delimiter=',', dtype=numpy.float32 , converters={4: conv},encoding='utf-8')
...
graphHist(loadD(),loadL())
graphScatter(loadD(),loadL())    
